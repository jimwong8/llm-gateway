...
func (r *PostgresRepository) MarkChunkFailed(ctx context.Context, chunkID, taskID, message string) error {
	...
	if n != 1 {
		return fmt.Errorf("chunk failure compare-and-swap failed")
	}
...
// AddTaskUsage records synthesis usage without changing completed_chunks.
func (r *PostgresRepository) AddTaskUsage(ctx context.Context, taskID string, inputTokens, outputTokens int, cost float64) error {
	_, err := r.db.ExecContext(ctx, `UPDATE long_context_tasks SET used_tokens=used_tokens+$1,estimated_cost=estimated_cost+$2,updated_at=NOW() WHERE id=$3`, inputTokens+outputTokens, cost, taskID)
	return err
}

func (r *PostgresRepository) GetAttemptCount(ctx context.Context, taskID string) (int, error) {
	var n int
	err := r.db.QueryRowContext(ctx, `SELECT count(*) FROM long_context_attempts WHERE task_id=$1`, taskID).Scan(&n)
	return n, err
}
