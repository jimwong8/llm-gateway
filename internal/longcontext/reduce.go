...
func ReduceClaims(inputs []ReducedClaim) (ReduceResult, error) {
	...
	for key, claims := range groups {
		...
		for _, c := range claims { quotes[strings.TrimSpace(c.Quote)] = true }
		if len(quotes) > 1 { result.Conflicts = append(result.Conflicts, ConflictGroup{Key:key, Claims:claims}) }
	}
	...
	if result.UncitedClaims > 0 { result.Status = "rejected"; return result, fmt.Errorf("reduce contains %d uncited claims", result.UncitedClaims) }
	return result, nil
}
