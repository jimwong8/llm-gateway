  ...
  return (
    <div>
      <div className="summary-card-grid">
        ...
        <SummaryMetricCard label={t('dashboard.promptTokens')} value={summary.prompt_tokens} />
        <SummaryMetricCard label={t('dashboard.completionTokens')} value={summary.completion_tokens} />
      ...
      {data.recent_api_keys && data.recent_api_keys.length > 0 && (
        <div className="page-surface">
          <h3 className="dashboard-section-title">{t('dashboard.myApiKeys')}</h3>
      ...
      <div className="page-surface">
        <h3 className="dashboard-section-title">{t('dashboard.usageTrends')}</h3>
        <DailyRequestsChart data={usageData} />
      </div>

      {modelData && modelData.length > 0 && (
        <div className="page-surface">
          <h3 className="dashboard-section-title">{t('dashboard.modelDistribution')}</h3>
          <ModelDistributionChart data={modelData} />
        </div>
      )}

      <div className="page-surface">
        <h3 className="dashboard-section-title">{t('dashboard.costTrend')}</h3>
      ...
      <div className="page-surface">
        <h3 className="dashboard-section-title">{t('dashboard.usageLogs')}</h3>
...
function UsageLogTable({ logs, t }: { logs: UserUsageLog[]; t: (key: string) => string }) {
  ...
  return (
    <table className="data-table">
      <thead>
        <tr>
          <th>{t('dashboard.logTime') || '时间'}</th>
          <th>Provider</th>
          <th>Model</th>
          <th>Tokens</th>
          <th>{t('dashboard.logCost') || '费用'}</th>
          <th>{t('dashboard.logStatus') || '状态'}</th>
          <th>{t('dashboard.logDuration') || '耗时'}</th>
        </tr>
      </thead>
      <tbody>
        {logs.map(l => (
