...
func SplitText(text string, chunkTokens, overlapTokens int) []struct {
	...
	if chunkRunes < 1 { chunkRunes = 1 }
	if overlapRunes >= chunkRunes { overlapRunes = chunkRunes / 4 }
	step := chunkRunes - overlapRunes
	out := make([]struct { Ordinal, StartChar, EndChar int; Content string }, 0, (len(runes)+step-1)/step)
	for start, ordinal := 0, 0; start < len(runes); ordinal++ {
		end := start + chunkRunes; if end > len(runes) { end = len(runes) }
		content := strings.TrimSpace(string(runes[start:end]))
		if content != "" { out = append(out, struct { Ordinal, StartChar, EndChar int; Content string }{ordinal, start, end, content}) }
