  ...
  return (
    <AppShell title={t('billing.title')} description={t('billing.description')}>
      ...
      <div className="page-surface" style={{ marginTop: '1rem' }}>
        <h3 className="billing-section-title">{t('billing.ledger')}</h3>
            ...
            <tbody>
              {(ledgerQuery.data.data ?? []).map(e => (
                <tr key={e.id}>
                  ...
                  <td>{e.type}</td>
                  <td className={e.amount < 0 ? 'billing-amount billing-amount--negative' : 'billing-amount billing-amount--positive'}>
