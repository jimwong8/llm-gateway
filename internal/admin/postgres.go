...
func (s *Store) RecentUsage(ctx context.Context, limit int) ([]UsageRow, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT tenant_id, user_id, request_id, model, provider, total_tokens, estimated_cost, created_at::text, COALESCE(cached_tokens, 0), COALESCE(cache_write_tokens, 0) FROM usage_events ORDER BY id DESC LIMIT $1`, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var row UsageRow
		if err := rows.Scan(&row.TenantID, &row.UserID, &row.RequestID, &row.Model, &row.Provider, &row.TotalTokens, &row.EstimatedCost, &row.CreatedAt, &row.CachedTokens, &row.CacheWriteTokens); err != nil {
			return nil, err
		}
		out = append(out, row)
	}
