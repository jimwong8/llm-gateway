...
// SetEmbedder attaches an embedding client to the in-memory L2 cache.
func (m *MemoryL2Cache) SetEmbedder(e EmbeddingClient) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.embedder = e
}

// SetPersistence enables periodic snapshot persistence for the cache.
func (m *MemoryL2Cache) SetPersistence(path string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.persistPath = path
}

// Save writes a snapshot of the in-memory cache to the persist path.
func (m *MemoryL2Cache) Save() error {
	m.mu.Lock()
	path := m.persistPath
	m.mu.Unlock()
	if path == "" {
	...
	data, err := json.Marshal(m.points)
	if err != nil {
		return err
	}
