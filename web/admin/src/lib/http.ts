...
export async function apiRequest<T>(
  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      // 无论请求是不是 admin 端点，401/403 都清理 token 并跳登录。
      // 之前只在 options.auth !== 'none' 时清理，导致 user token 访问
      // /api/user/dashboard 拿到 401 后 token 仍留在 sessionStorage，
      // 用户卡死在 dashboard 页面看到 '首页概览加载失败' 而不是被引导回登录。
      clearToken()
