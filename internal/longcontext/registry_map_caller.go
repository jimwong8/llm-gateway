...
func (c RegistryMapCaller) CallMap(ctx context.Context, channel string, chunk Chunk) (string, error) {
	...
	if strings.TrimSpace(channel) == "" {
		return "", fmt.Errorf("map channel is required")
	}
	model := strings.TrimSpace(c.Model)
	if model == "" {
		return "", fmt.Errorf("map model is required")
	}
	...
	resp, err := c.Registry.ChatCompletion(ctx, channel, providers.ChatCompletionRequest{
		Model: model, MaxTokens: maxTokens, ReasoningEffort: c.ReasoningEffort,
	...
	if len(resp.Choices) == 0 {
		return "", fmt.Errorf("map provider returned no choices")
	}
