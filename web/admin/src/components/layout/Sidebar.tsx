...
export const SidebarLayout = React.memo(function SidebarLayout({
  mobile = false,
  open = false,
  onClose,
  currentPath = '',
  ...
  // mobile sidebar 忽略 collapsed 状态
    ...
    >
      ...
      <div className="app-sidebar__footer">
        {!mobile && onToggleCollapse && (
          <button
            type="button"
            className="nav-item nav-item--toggle-collapse"
            onClick={onToggleCollapse}
            aria-label={isCollapsed ? '展开侧边栏' : '折叠侧边栏'}
            title={isCollapsed ? '展开侧边栏' : '折叠侧边栏'}
          >
            <span className="nav-item__label">{isCollapsed ? '»' : '«'}</span>
          </button>
        )}
        <button
          type="button"
