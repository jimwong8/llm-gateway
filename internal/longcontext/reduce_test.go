...
func TestReduceClaimsDeduplicatesAndPreservesQuotes(t *testing.T) {
	...
	if result.Claims[0].Quote != "revenue increased 10%" {
		t.Fatal("quote was not preserved")
	}
	if result.CitationCoverage != 1 {
		t.Fatalf("coverage=%v", result.CitationCoverage)
	}
...
func TestReduceClaimsGroupsConflictingQuotes(t *testing.T) {
	...
	if len(result.Conflicts[0].Claims) != 2 {
		t.Fatal("conflict claims lost")
	}
}
...
func TestReduceClaimsRejectsUncitedClaim(t *testing.T) {
	...
	if err == nil {
		t.Fatal("accepted uncited claim")
	}
	if result.Status != "rejected" || result.UncitedClaims != 1 {
